...
public class Client_Cosmos {
    ...
    public void buy(String customerId, long itemNum, int priceOffered, int unitsAdded, int buyTime, String dis, String dead){

        if (!shoppingCart.customerExists(customerId)) { shoppingCart.addCustomerToTable(customerId); }
        ...
        shoppingCart.put(customerId, item);
    }
     ...
     */
    public ArrayList<JSONObject> listCart(String customerId){
        if (!shoppingCart.customerExists(customerId)) { shoppingCart.addCustomerToTable(customerId); }

        JSONObject cart = shoppingCart.getShoppingCart(customerId);
...
//        System.out.println("listing Cart");
    ...
    public void updateCount(String customerId, int change, long itemNumber){
        if (!shoppingCart.customerExists(customerId)) { shoppingCart.addCustomerToTable(customerId); }
        shoppingCart.update(customerId, change, itemNumber);
    }
    ...
    public void checkoutAndPay(String customerId){
        if (shoppingCart.customerExists(customerId)) {
